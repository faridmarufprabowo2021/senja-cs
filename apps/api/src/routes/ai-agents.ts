...
export function mapAiAgent(agent: any) {
  return {
    ...
    quietHoursEnabled: agent.quietHoursEnabled ?? true,
    quietHoursStart: agent.quietHoursStart || "22:00",
    quietHoursEnd: agent.quietHoursEnd || "08:00",
    quietHoursTz: agent.quietHoursTz || "Asia/Jakarta",
    ...
    async (request) => {
      const body = z
        .object({
          ...
          quietHoursTz: z.string().max(40).optional(),
      ...
      const agent = await prisma.aiAgent.create({
        data: {
          ...
          quietHoursTz: body.quietHoursTz || "Asia/Jakarta",
